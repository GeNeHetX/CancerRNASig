...
# load and func
{
  ...
  addgs <- function(gsobj, geneset, type, src, id, addIDtoNameList = T) {
    if (length(type) == 1) {
      type <- rep(type, length(geneset))
    }
    if (length(src) == 1) {
      src <- rep(src, length(geneset))
    }
    if (length(id) == 1) {
      id <- rep(id, length(geneset))
    }
...
# PDAC
{
  ...
  baileyMarkG <- lapply(list(
    ...
    Squamous = file.path(.refpath, "PDAC_SquamousvRest_suptab2.txt")
  ), function(f) {
  ...
  # Hwang
  {
    ...
    b <- sub("^X.+", "", colnames(a))
    for (i in 1:length(b)) if (b[i] == "") b[i] <- b[i - 1]
...
# IMMUNE
{
  ...
  ImmuneL <- list(
    ICKrelated = toupper(scan(file.path(.refpath, "YLickrelated.txt"), what = "character", sep = "\n")),
...
# CCK: Cjholangiocarcnoma
{
  ...
  Biclassgenes <- list(
    PROLIFGenes = scan(file.path(.refpath, "proliferationGenes.txt"), what = "character"),
...
# DrugBank
{
  ...
  drugbank <- setNames(lapply(fils, \(f){
    scan(file.path(.refpath, "drugBank", f), what = "character", sep = "\n")
  }), sub(".txt$", "", fils))
}
...
# K.Mulder;PMID:34331874 Macrophages
{
  ...
  cluster_annot <- list(
    "Mono_CD16pos"    = c(1, 5),
    "Mono_CD16neg"    = c(15, 12, 8, 4),
    "Macro"           = c(2, 3, 6, 7, 11, 13, 16, 17),
    "Prolif"          = c(10),
    "T_cell_doublets" = c(9),
    "DC2_DC3"         = c(14)
  )

  rename_cluster <- function(cl) {
    annot <- names(cluster_annot)[sapply(cluster_annot, function(x) cl %in% x)]
    if (length(annot) == 0) {
      return(NA)
    }
    return(paste0(annot, "_cluster", cl))
  }

  mulder_deg$annotation <- sapply(mulder_deg$cluster, rename_cluster)
  mulder_genesets <- split(mulder_deg$gene, mulder_deg$annotation)
}
...
gsignatures <- gsignatures %>%
  ...
  addgs(geneset = grunwaldsig, type = "Stroma pancreas", src = "Grunwald.etal;PMID:34644529", id = "Stroma.Grunwald21") %>%
